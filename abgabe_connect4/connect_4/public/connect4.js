...
async function setServerButtons() {
  serverStatus = await serverAccessible()
  const children = document.getElementById("controlsServer").children
  for (let i = 0; i < children.length; i++) {
    children[i].disabled = !serverStatus;
  }
}
...
function loadStateBrowser() {
  ...
  if (savedState !== null) {
    for (let ix = 0; ix < state.board.length; ix++) {
      for (let iy = 0; iy < state.board[0].length; iy++) {
        state.board[ix][iy] = savedState.board[ix][iy]
      }
    }
    state.playerTurn = savedState.playerTurn
    pushStateToHistory()
  }
  showBoard();
}
...
async function serverAccessible() {
  try {
...
function loadStateFromHistory(){
  ...
  oldState = stateSeq.pop()
  
  for (let ix = 0; ix < state.board.length; ix++) {
    for (let iy = 0; iy < state.board[0].length; iy++) {
      state.board[ix][iy] = oldState.board[ix][iy]
    }
  }
  state.playerTurn = oldState.playerTurn
